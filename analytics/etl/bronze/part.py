...
class PartBronze(Table):
    def __init__(
        ...
        storage_path: str = 's3://duckdb-bucket-tpch/bronze/part',
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        input_relation.create_view("tmp_part_input", replace=True)
    ...
    def read(self, partition_values: Optional[Dict[str, str]] = None) -> ETLDataSet:
        ...
        if partition_values:
            ...
            all_data.create_view("all_part_partitions", replace=True)

            latest = self.conn.execute("SELECT max(etl_inserted) FROM all_part_partitions").fetchone()[0]
            filtered = self.conn.from_query(
                f"SELECT * FROM all_part_partitions WHERE etl_inserted = '{latest}'"
            )
