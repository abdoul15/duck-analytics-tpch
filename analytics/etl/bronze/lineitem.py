...
class LineItemBronze(Table):
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        input_relation.create_view("tmp_lineitem_input", replace=True)

        etl_date = datetime.now().isoformat()

        transformed_query = f"""
            SELECT *, '{etl_date}' AS etl_inserted
            FROM tmp_lineitem_input
        """
    ...
    def read(self, partition_values: Optional[Dict[str, str]] = None) -> ETLDataSet:
        ...
        if partition_values:
            ...
            all_data.create_view("all_lineitem_partitions", replace=True)

            latest = self.conn.execute("SELECT max(etl_inserted) FROM all_lineitem_partitions").fetchone()[0]
            filtered = self.conn.from_query(
                f"SELECT * FROM all_lineitem_partitions WHERE etl_inserted = '{latest}'"
            )
