...
class PartSuppBronze(Table):
    def __init__(
        ...
        storage_path: str = 's3://duckdb-bucket-tpch/bronze/partsupp',
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        input_relation.create_view("tmp_partsupp_input", replace=True)

        etl_date = datetime.now().isoformat()

        transformed_query = f"""
            SELECT *, '{etl_date}' AS etl_inserted
            FROM tmp_partsupp_input
        """
    ...
    def read(self, partition_values: Optional[Dict[str, str]] = None) -> ETLDataSet:
        ...
        if partition_values:
            ...
            all_data.create_view("all_partsupp_partitions", replace=True)

            latest = self.conn.execute("SELECT max(etl_inserted) FROM all_partsupp_partitions").fetchone()[0]
            filtered = self.conn.from_query(
                f"SELECT * FROM all_partsupp_partitions WHERE etl_inserted = '{latest}'"
            )
